...
def train(args):
    ...
    for i in range(args.epochs):
        ...
        for j in trange(trainset.batch_count):
            _, sentence_tokens, sentence_poses, sentence_adjs, lengths, masks, aspect_tags, _, tags = trainset.get_batch(j)
...
def eval(model, dataset, args):
    ...
    with torch.no_grad():
        ...
        for i in range(dataset.batch_count):
            sentence_ids, sentence_tokens, sentence_poses, sentence_adjs, lengths, mask, aspect_tags, _, tags = dataset.get_batch(i)
            prediction = model.forward(sentence_tokens,sentence_poses,sentence_adjs,lengths, mask)
